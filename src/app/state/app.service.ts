...
export class AppService {
  ...
  initializeConnection(): Observable<unknown> {
    ...
    const key = v4();
    return this.httpChannelService
      .post<ConnectResponse>("/connect/xray", {
        country: country?.id,
        key,
      })
      .pipe(
  ...
  async runCore(): Promise<unknown> {
    ...
    if (platform === "windows") {
      return this.commandChannelService
        .spawn(this.appQuery.winCoreExePath, [
          "run",
          `-config=${this.appQuery.winCoreConfigPath}`,
        ])
        .then((pid) => {
  ...
  async applyNetworkProxy(): Promise<unknown> {
    ...
    if (platform === "windows") {
      ...
      return this.commandChannelService.execute(cmd, "user");
    }
  ...
  async writeConfig(client_config: string): Promise<unknown> {
    ...
    const config =
      platform === "windows"
        ? this.adjustWindowsConfig(client_config)
        : this.adjustMacosConfig(client_config);

    if (platform === "windows") {
      return this.commandChannelService
        .writeFileSync(this.appQuery.winCoreConfigPath, config)
        .catch((error) => {
          console.log(error);
        });
    }
  ...
  async reset(): Promise<void> {
    return this.disableNetworkProxy()
      .then(() => {
